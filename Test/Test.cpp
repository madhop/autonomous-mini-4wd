...
for(;;){
  ...
  Mat filtered_w, filtered_y, filtered;
  //White Filter
  inRange(src, Scalar(200, 200, 200), Scalar(255, 255, 255), filtered_w);
  //Yellow Filter
  cvtColor(src, filtered_y, COLOR_RGB2HSV);
  inRange(filtered_y, Scalar(50, 50, 50), Scalar(110, 255, 255), filtered_y);
  //blend
  addWeighted( filtered_y, 1, filtered_w, 1, 0.0, filtered);
  ...
  Canny( filtered, detected_edges, lowThreshold, lowThreshold*ratio, canny_kernel ); /// Canny detector
...
/*
  ...
  bitwise_and(detected_edges, mask, detected_edges);

  //Hugh Transform for Line detection
  vector<Vec4i> lines;
  HoughLinesP(detected_edges, lines, 1, CV_PI/180, 50, 50, 200 ); //original: HoughLinesP(detected_edges, lines, 1, CV_PI/180, 10, 50, 200 ); http://docs.opencv.org/2.4/doc/tutorials/imgproc/imgtrans/hough_lines/hough_lines.html
