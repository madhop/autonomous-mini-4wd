...
#define max_bad_curves 1
#define min_good_curves 1
#define order 2
#define min_barycenters 3 //in realtà andrebbe messo come ratio e diviso per n_rect
...
Mat perspectiveTransform(Mat mat, vector<Point2f> perspTransfInPoints, vector<Point2f> perspTransfOutPoints){
  ...
  */
  ...
  displayImg("warpPerspective", mat);
  return mat;
}
...
vector<float> polyFit(vector<Point> points,Mat mat){
  vector<float> beta = vector<float>();
  int width = mat.size().width;
  int height = mat.size().height;
  if(points.size() >= order + min_barycenters){
...
bool collideCheck(vector<Point> curveRectCentersA, vector<Point> curveRectCentersB, int rect_height, int rect_width){
  for(int i = 0; i < n_rect; i++){
    vector<Point> rectA = computeRect(curveRectCentersA[i], rect_width, rect_height);
    vector<Point> rectB = computeRect(curveRectCentersB[i], rect_width, rect_height);
    if(curveRectCentersA[i].x > 0 && curveRectCentersB[i].x > 0){
      if((rectB[0].x >= rectA[0].x && rectB[0].x <= rectA[3].x) || (rectB[3].x >= rectA[0].x && rectB[3].x <= rectA[3].x) ){
        return true;
      }
    }
...
void updateState(bool curve_ok, vector<Point> &fittedCurve, bool &some_curve, int &curve_bad_series, int &curve_ok_series, vector<Point> &lastFittedCurve,
   vector<Point> &lastOkFittedCurve, vector<Point> &lastOkCurveRectCenters, vector<Point> &curveRectCenters, vector<float> beta, vector<float> &lastOkBeta){
  //Update states
...
Point nextRectCenter(int y, vector<Point> points, Mat mat){
  vector<float> beta = polyFit(points,mat);
...
int findCurvePoints(bool &some_curve, vector<Point> &rectCenters, vector<Point> & barycenters, int pos, Mat wip, int width, int height, int rect_offset, int rect_height, int rect_width, Mat rectangles, vector<Point> &lastOkRectCenters, vector<float> &beta, int offset, vector<bool> &barsFound){ //pos: 0=left, 1=right
  if(some_curve == false){
...
drawRect(rect, rect_color, height, rectangles);
  ...
  do{//for(int i=0;i<n_rect;i++){
    ...
    if(barycenters.size() >= order + min_barycenters && i<n_rect-1){ // if more than n barycenters where found, find the next center fitting a parabola
      nextCenter = nextRectCenter(rectCenters[i+1].y, barycenters, wip);
...
int detectLanes(Mat src, vector<Point> &lastOkFittedRight, vector<Point> &lastOkFittedLeft, vector<Point> &lastOkRightRectCenters,
  ...
  Point &vanishing_point_avg, vector<bool> leftBarsFound, vector<bool> rightBarsFound){
    ...
    findCurvePoints(some_left, leftRectCenters, leftBarycenters, 0, wip, width, height, rect_offset, rect_height, rect_width, rectangles, lastOkLeftRectCenters, lastOkBetaLeft, mask_offset, leftBarsFound);
    cout << "right curve" << endl;
    findCurvePoints(some_right, rightRectCenters, rightBarycenters, 1, wip, width, height, rect_offset, rect_height, rect_width, rectangles, lastOkRightRectCenters, lastOkBetaRight, mask_offset, rightBarsFound);
    ...
    if(leftBarycenters.size() >= order + min_barycenters){
      leftBeta = polyFit(leftBarycenters,wip);
      fittedLeft = computePoly(leftBeta, height);
    }
    if(rightBarycenters.size() >= order + min_barycenters){
      rightBeta = polyFit(rightBarycenters,wip);
      fittedRight = computePoly(rightBeta, height);
    }
    // check whether the lanes collide
    bool collide = collideCheck(leftRectCenters, rightRectCenters, rect_height, rect_width);
    if(!collide){
      //* Classify Curves *
      cout << "Classify left" << endl;
      left_ok = classifyCurve(fittedLeft, some_left, left_similar_series, left_bad_series, left_ok_series, lastFittedLeft,
         lastOkFittedLeft, lastOkLeftRectCenters, leftRectCenters, leftBeta, lastOkBetaLeft);
      cout << "Classify right" << endl;
      right_ok = classifyCurve(fittedRight, some_right, right_similar_series, right_bad_series, right_ok_series, lastFittedRight,
        lastOkFittedRight, lastOkRightRectCenters, rightRectCenters, rightBeta, lastOkBetaRight);
    }else{
      cout << "*****collide!!" << endl;
      left_ok = false;
      right_ok = false;
    ...
    updateState(left_ok, fittedLeft, some_left, left_bad_series, left_ok_series, lastFittedLeft,
      lastOkFittedLeft, lastOkLeftRectCenters, leftRectCenters, leftBeta, lastOkBetaLeft);
    updateState(right_ok, fittedRight, some_right, right_bad_series, right_ok_series, lastFittedRight,
      lastOkFittedRight, lastOkRightRectCenters, rightRectCenters, rightBeta, lastOkBetaRight);
...
Mat computeBinaryThresholding(Mat src){ //thresholding with just adaptive threshold on gray scale image
  ...
  int meanIntensity = mean(wip)[0];

  inRange(wip,meanIntensity + 2/float(5)*meanIntensity,255, wip); //Scalar(150, 150, 150)
  adaptiveThreshold(wip,wip,255,ADAPTIVE_THRESH_GAUSSIAN_C,THRESH_BINARY,105,0); //55, -20
