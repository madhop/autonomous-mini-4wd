...
int findCurvePoints(bool &some_curve, vector<Point> &rectCenters, vector<Point> & barycenters, int pos, Mat wip, int width, int height, int rect_offset, int rect_height, int rect_width, Mat rectangles, vector<Point> &lastOkRectCenters, vector<float> &beta, int offset){ //pos: 0=left, 1=right
  ...
  for(int i=0;i<n_rect;i++){
    ...
    Point bar = computeBarycenter(rect ,wip);
    int poly_y1 = (-beta[1]+sqrt(pow(beta[1],2)-(4*beta[2]+( (beta[0]-bar.x ) +(offset/2) ) )))/(2*beta[2]);
    int poly_y2 = (-beta[1]+sqrt(pow(beta[1],2)-(4*beta[2]+( (beta[0]-bar.x ) -(offset/2) ) )))/(2*beta[2]);
    int poly_x1 = beta[0]+(offset/2) + beta[1]*bar.y + beta[2]*pow(bar.y,2);
    int poly_x2 = beta[0]-(offset/2) + beta[1]*bar.y + beta[2]*pow(bar.y,2);

    if(bar.x < poly_x1 && bar.x > poly_x2 && bar.y < poly_y2 && bar.y > poly_y1){
...
int findCurvePoints(bool &some_curve, vector<Point> &rectCenters, vector<Point> & barycenters, int pos, Mat wip, int width, int height, int rect_offset, int rect_height, int rect_width, Mat rectangles, vector<Point> &lastOkRectCenters){ //pos: 0=left, 1=right
  ...
  if(some_curve == false){
    rectCenters = vector<Point>();
    //First rectangle
    firstX = findHistAcc(wip, pos); //0 means left
    if(firstX == -1){  //in caso non trovi il massimo
      firstX = width/4;
    }
  }
  else{
    firstX = lastOkRectCenters[0].x;
  }
...
int detectLanes(Mat src, vector<Point> &lastOkFittedRight, vector<Point> &lastOkFittedLeft, vector<Point> &lastOkRightRectCenters,
  ...
  if(perspTransfInPoints.size()>0){ //If vanishing point has been found
    perspTransfOutPoints.push_back(Point2f( (width/2)-(width/5), (height/2)+(height/2) ));
    perspTransfOutPoints.push_back(Point2f( (width/2)-(width/5), (height/2)-(height/5) ));
    perspTransfOutPoints.push_back(Point2f( (width/2)+(width/5), (height/2)-(height/5) ));
    perspTransfOutPoints.push_back(Point2f( (width/2)+(width/5), (height/2)+(height/2) ));
    /*old school
